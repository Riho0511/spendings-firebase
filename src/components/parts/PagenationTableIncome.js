...
import { deleteIncome } from '../../redux/spendings/operations';
...
const PagenationTableIncome = (props) => {
  ...
  const emptyRows = rowsPerPage - Math.min(rowsPerPage, income.length - page * rowsPerPage);
  ...
  return (
    <TableContainer component={Paper}>
      <Table className={classes.table} aria-label="custom pagination table">
        ...
        <TableBody>
          {(rowsPerPage > 0
            ? income.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
            : income
          ).map(row => (
            <TableRow key={row.incomeId}>
              ...
              <TableCell align='center'>
                <IconButton onClick={() => dispatch(push('/edit/income/'+ row.incomeId))}>
                  <EditIcon />
                </IconButton>
                <IconButton onClick={() => dispatch(deleteIncome(row.incomeId))}>
                  <DeleteIcon />
                </IconButton>
              </TableCell>
            </TableRow>
...
export default PagenationTableIncome;